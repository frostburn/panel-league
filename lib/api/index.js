...
class API {
  ...
  addEvent(uuid, data) {
    ...
    if (engine) {
      const result = engine.validateEvent(data);
...
function installAPI(app, server) {
  ...
  app.get('/api/play/:adapter/', (req, res) => {
    ...
    if (req.query.poll && !state.canPlay) {
      // Status needs to be pollable so that the client knows when to stop.
      const result = { canPlay: false, status: state.status };
  ...
  app.post('/api/play/:adapter/', (req, res) => {
    ...
    // Overwrite time and player to prevent abuse
    event.player = state.player;
    event.time = state.time;
    const response = api.addEvent(req.params.adapter, event);
