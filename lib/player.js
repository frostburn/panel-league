...
class Player {
  ...
  installEventListeners() {
    ...
    this.socket.on('game list', () => {
      this.socket.emit('game list', {
        games: Object.keys(this.server.games).map((id) => {
  ...
  createGame(gameModeName) {
    // Currently we support only game mode called 'vs', even though that is not
    // very descriptive name.
    if (gameModeName !== 'vs') {
      this.socket.emit('client error', { message: `Unrecognized game mode: ${gameModeName}` });
      return;
    }
