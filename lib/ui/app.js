...
export default class App {
  ...
  onCreateNewGame({ mode }) {
    if (this.socket) {
      const game = gameFactory(mode, this.keyBindings[mode]);
  ...
  onViewAccount() {
    setChildren(this.el, this.account = new Account());
  }
  ...
  onSetKeyBindings({ keyBindings, gameMode }) {
    if (this.socket) {
      this.socket.emit('session event', {
        sessionId: this.sessionId,
        type: 'keyBindings',
  ...
  handleSessionEvent(event) {
    if (event.type === 'nick') {
      ...
      if (event.gameMode == null) {
        this.keyBindings = event.value;
      } else {
        this.keyBindings[event.gameMode] = event.value;
      }
    }
  }
