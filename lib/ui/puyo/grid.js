...
export default class Grid {
  constructor(state, hidePreviewGhosts, name) {
    ...
    this.el = el('',
      el('.column',
        ...
        el('.display', el('span', name)),
      ),
      el('.column-narrow',
        this.dealPreview = new DealPreview({ deals: deals.slice(1) }),
        el('.display narrow',
          this.waitingDisplay = el('span.slow-fade-out.flashing', 'Waiting...')
        ),
        el('.display narrow',
          el('span', 'Time '),
          this.timeControlDisplay = el('span', '∞')
        ),
        el('.display narrow',
          el('span', 'Score'),
          el('br'),
