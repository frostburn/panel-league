...
export default class PuyoDuelGame extends PuyoGameBase {
  ...
  onEstablishConnection({ socket, player, game, timeControl, spectating, metadata }) {
    ...
    this.playerGrid = new Grid(state, spectating, metadata.players[this.player].name);
    state = parentState.childStates[opponent];
    state.deals = parentState.deals.slice(state.dealIndex, state.dealIndex + parentState.numDeals);
    this.opponentGrid = new Grid(state, true, metadata.players[opponent].name);
    setChildren(this.el, [this.playerGrid, this.opponentGrid]);
    setChildren(this.el, [this.playerGrid, this.opponentGrid, this.footerEl]);
    if (spectating) {
