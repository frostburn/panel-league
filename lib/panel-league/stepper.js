...
// FIXME: Import entire library instead. Why 'newBlock' is contained in utility
// library?
...
class StandardStepper {
  ...
  initializeState(options) {
    ...
    const state = {
      ...
      RNG: (() => {
        // This is example of poor API design. All this should be available as
        // function.
        const x = new JKISS31();
...
class ScoringStepper extends StandardStepper {
  ...
  getScore(state, effect) {
    if (!(state.scoringSystem in scoring)) {  // why?
...
// FIXME: Just export the factory function instead and hide everything else.
