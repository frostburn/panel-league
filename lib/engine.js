...
const tetris = require('./tetris/stepper.js');
...
function factory(name) {
  let stepper = panelLeague.factory(name);
  if (stepper) {
    return stepper;
  }
  stepper = tetris.factory(name);
  if (stepper) {
    return stepper;
  }
  throw new Error("Unknown stepper");
}
...
class GameEngine {
  ...
  rebase(earlierState) {
    ...
    this.initialState = earlierState;
